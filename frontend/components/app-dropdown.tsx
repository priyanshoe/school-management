...
export function DropdownClass({ defaultClass, setClass }: DropdownProp) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="default" className={`w-1/2 bg-white hover:bg-[#f8f8f8] text-black rounded-full border  focus-visible:border-chart-4 focus-visible:ring-chart-4 focus-visible:ring-[1px] data-[state=open]:border-green-500`}>
          { selectedClass || "Class" }
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-56">
        ...
        <DropdownMenuRadioGroup value={selectedClass} onValueChange={handleChange}>
          {
            (classes) &&
            classes.map((item) => (
              <DropdownMenuRadioItem key={item.class_id} value={item.name}>{item.name}</DropdownMenuRadioItem>
            ))
          }
        </DropdownMenuRadioGroup>
      </DropdownMenuContent>
