...
async function signUpTeacher(req, res) {
    try {
        ...
        if (!jwtkey) return res.status(500).json({ message: "SignIn Success \n error in genrating token \n try sign-in", data: created.insertId });
        const token = jwt.sign({ id: created.insertId, role: 'teacher' }, jwtkey, { expiresIn: '7d' })
        ...
        return res.status(500).json({ message: "SignIn failed", error: JSON.stringify(error) });
    }
}
