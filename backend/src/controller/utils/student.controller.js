...
async function updateStudent(req, res) {
    ...
    try {
        ...
        if(subjects.length>0){
            ...
            const [insert_subjects] = await connection.query("insert into subjects_students (student_id,subject_id) values ?",[values])            
            if(insert_subjects.affectedRows === 0) throw new Error("Subjects on saved")
            await connection.commit()
        }
